...
def py_focal_loss_with_prob(pred,
    ...
    target = target.type_as(pred)
    pt = ((1 - pred)/pred) * target + (pred/(1 - pred)) * (1 - target)
...
class MFocalLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0,
                 activated=False):
        """`MFocal Loss <https://arxiv.org/abs/1708.02002>`_
        ...
        Args:
            ...
            gamma (float, optional): The gamma for calculating the modulating
                factor. Defaults to 2.0.
            alpha (float, optional): A balanced form for MFocal Loss.
                Defaults to 0.25.
            reduction (str, optional): The method used to reduce the loss into
                a scalar. Defaults to 'mean'. Options are "none", "mean" and
                "sum".
        ...
        assert use_sigmoid is True, 'Only sigmoid MFocal loss supported now.'
        self.use_sigmoid = use_sigmoid
        self.gamma = gamma
        self.alpha = alpha
        self.reduction = reduction
